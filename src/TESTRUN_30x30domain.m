...
A = AvulsionModel;                    % Tested - GOOD

% Adjust the topography so that the steepest-descent algorithm creates a
% diagonal channel starting at the top-left and ending at the bottom-right
T = A.topographyLow + .5*(ones(30) - eye(30));  % Tested - GOOD
A.setTopographyLow(T);                % Tested - GOOD
A.setTopographyHigh(T);               % Tested - GOOD
...
TotalSedimentOut(i) = sum(FluxOutput(1000*(i-1)+(1:1000))) * A.timeStepSize;
  ...
  % get slope information - neighbor #5 on the chart in getSlopes.m
  temp = A.getSlopes([1,1]);   % Slope at input
  s1(i) = temp(5);
  temp = A.getSlopes([15,15]);  % Slope at middle of domain
  s2(i) = temp(5);
  temp = A.getSlopes([29,29]);  % Slope at output
  s3(i) = temp(5);
